...
async def broadcast(bot, message):
    ...
    sts = await message.reply_text('Bʀᴏᴀᴅᴄᴀsᴛɪɴɢ Yᴏᴜʀ ᴍᴇꜱꜱᴀɢᴇs...')
...
async def remove_junkuser__db(bot, message):
    ...
    time_taken = datetime.timedelta(seconds=int(time.time()-start_time))
    await sts.delete()
    await bot.send_message(message.chat.id, f"Terminé:\nTerminé in {time_taken} seconds.\n\nUtilisateurs totaux {total_users}\nTerminé: {done} / {total_users}\nBlocked: {blocked}\nDeleted: {deleted}")
...
async def broadcast_group(bot, message):
    ...
    async for group in groups:
        ...
        if not done % 20:
            await sts.edit(f"Broadcast in progress:\n\nTotal Groups {total_groups}\nTerminé: {done} / {total_groups}\nSuccès: {success}\nDeleted: {deleted}")    
    time_taken = datetime.timedelta(seconds=int(time.time()-start_time))
    await sts.delete()
    try:
        await message.reply_text(f"Broadcast Terminé:\nTerminé in {time_taken} seconds.\n\nTotal Groups {total_groups}\nTerminé: {done} / {total_groups}\nSuccès: {success}\nDeleted: {deleted}\n\nFiled Reson:- {failed}")
    except MessageTooLong:
        with open('reason.txt', 'w+') as outfile:
            outfile.write(failed)
        await message.reply_document('reason.txt', caption=f"Terminé:\nTerminé in {time_taken} seconds.\n\nTotal Groups {total_groups}\nTerminé: {done} / {total_groups}\nSuccès: {success}\nDeleted: {deleted}")
...
async def junk_clear_group(bot, message):
    ...
    try:
        ...
        with open('junk.txt', 'w+') as outfile:
            outfile.write(failed)
        await message.reply_document('junk.txt', caption=f"Terminé:\nTerminé in {time_taken} seconds.\n\nTotal Groups {total_groups}\nTerminé: {done} / {total_groups}\nDeleted: {deleted}")
        os.remove("junk.txt")
