...
async def git(Kashmira, message):
    pablo = await message.reply_text("`Traitement...`")
    ...
    if lool.get("total_count") == 0:
        await pablo.edit("File non trouvé")
        return
    else:
        lol = lool.get("items")
        qw = lol[0]
...
async def github_user(Kashmira, message):
    ...
    if len(message.command) == 1:
        await pablo.edit("Veuillez fournir un nom d'utilisateur GitHub. Exemple : `/github username`")
        return
    ...
    if r.status_code != 200:
        await pablo.edit(f"GitHub user `{username}` non trouvé.")
        return
