...
async def handle_chat_model(message: Message, model_name: str, model):
    ...
    if not prompt:
        await message.reply_text("Veuillez fournir un prompt après la commande.")
        return
...
async def geminivision_handler(client: Client, message: Message):
    if not (message.reply_to_message and message.reply_to_message.photo):
        await message.reply_text("Veuillez répondre à une image avec la commande /geminivision et un prompt.")
        return
    ...
    status_message = await message.reply_text("Traitement your image, please wait...")
...
async def upscale_handler(client: Client, message: Message):
    if not (message.reply_to_message and message.reply_to_message.photo):
        await message.reply_text("Veuillez répondre à l'image que vous souhaitez agrandir avec la commande /upscale.")
        return
