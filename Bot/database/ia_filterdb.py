...
from utils import get_paramètres, save_group_paramètres, temp, get_status
...
async def save_file(bot, media):
  ...
  try:
    ...
    logger.exception('Erreur survenue while saving file in database')
    return False, 2
  else:
    try:
      await file.commit()
...
async def get_search_results(chat_id, query, file_type=None, max_results=10, offset=0, filter=False):
    ...
    if chat_id is not None:
        paramètres = await get_paramètres(int(chat_id))
        try:
            ...
            await save_group_paramètres(int(chat_id), 'max_btn', False)
            paramètres = await get_paramètres(int(chat_id))
            if paramètres['max_btn']:
                max_results = 10
            else:
...
async def send_msg(bot, filename, caption): 
    try:
        ...
        if await add_name(OWNERID, filename):
            ...
            btn = [[InlineKeyboardButton('Recherché', url=f"https://telegram.me/{temp.U_NAME}?start=getfile-{filenames}")]]
