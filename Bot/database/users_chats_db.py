...
class Database:
    ...
    async def get_banni(self):
        users = self.col.find({'ban_status.is_banni': True})
    ...
    async def update_paramètres(self, id, paramètres):
        await self.grp.update_one({'id': int(id)}, {'$set': {'paramètres': paramètres}})
            
    async def get_paramètres(self, id):
        default = {
        ...
        if chat:
            return chat.get('paramètres', default)
        return default
