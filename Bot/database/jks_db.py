...
async def get_user_access(user_id: int) -> dict:
    """
    Retourne l'état d'accès d'un utilisateur.
    Vérifie d'abord le plan premium, ensuite la session gratuite.
    Retourne :
      {
        "has_access": bool,
        "access_type": "premium" | "session" | "none",
        "plan": str,
        "session_expiry": datetime | None,
    ...
    # Priorité 2 : session gratuite (pub)
...
async def grant_free_session(user_id: int) -> datetime.datetime:
    """
    Attribue une session gratuite de 1 heure après pub vue.
    Appelé par le handler web_app_data.
    """
...
async def grant_premium_plan(user_id: int, plan: str) -> datetime.datetime:
    """
    Attribue un plan premium. Utilisé par l'admin via /premium ou après paiement TON.
    """
...
async def log_search(user_id: int, query: str) -> None:
    ...
    Appelé à chaque fois qu'un utilisateur envoie un texte de recherche dans le bot.
    """
...
async def get_recent_searches(user_id: int, limit: int = 10) -> list:
    """
    Retourne les N dernières recherches d'un utilisateur, du plus récent au plus ancien.
    """
...
async def get_search_stats(user_id: int) -> dict:
    ...
    - total_searches  : int
    - week_searches   : int
    - daily_searches  : list[int] — 7 valeurs Lundi → Dimanche semaine courante
    """
    now = datetime.datetime.utcnow()
...
def format_expiry(expiry: datetime.datetime) -> str:
    if not expiry:
        return "N/A"
    return expiry.strftime("%d %b %Y à %H:%M UTC")
